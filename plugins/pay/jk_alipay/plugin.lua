    ...
    if reportApps[packageName] then
        ...
        for i, v in ipairs(reportApps[packageName]) do
            ...
            if v.Code == msg['channel_code'] then
                ...
                local contentMatched = helper.regexp_match(msg.content, v.ContentReg)
                if titleMatched and contentMatched then
                    -- 调用正则
                    local matched, matchGroups
                    -- 判断金额是否在标题中
                    if v.AmountInTitle then
                        matched, matchGroups = helper.regexp_match_group(msg.title, v.TitleReg)
                    else
                        matched, matchGroups = helper.regexp_match_group(msg.content, v.ContentReg)
                    end

...
plugin = {
    info = {
        ...
        options = {
            ...
            -- 定时任务
            crontab_list = {
                { crontab = "*/180 * * * * *", fun = "usdt_cny_rate", name = "同步USDT Cny 汇率" },
                { crontab = "*/180 * * * * *", fun = "usdt_trx_rate", name = "同步USDT Trx 汇率" },
            },
            -- 配置项
            options = {
                {
...
function plugin.cron(pAccountInfo, pPluginOptions)
    ...
    if error_message ~= nil then
        return json.encode({
            err_code = 500,
            err_message = string.format('请求响应错误 请求地址:%s 返回消息:%v 错误信息: %v',uri, response, error_message)
    ...
    if returnInfo == nil then
        return json.encode({
            err_code = 500,
            err_message = string.format('未能获取到订单数据1 请求地址:%s 返回数据:%v',uri, response.body)
        })
    end
